...
//je crée une palette je prends son id, je genère size couleurs je leur donne l'id de la palette
//je fais toutes les palettes comme favoritesf
...
class _GeneratePaletteScreenState extends State<GeneratePaletteScreen> {
  ...
  var lockList = [];
  Color selectedColor;
  My_Palette addedPalette;

  void getindex(Color c) {
    selectedColor = c;
  }
  int get_category_id(String type){
    int id;
    if(type==wtones)id=1;
    if(type==ctones)id=2;
    if(type==dtones)id=3;
  ...
  List initialisePalette(int n, String types) {
    ...
    if (types == wtones) {
      for (int i = 0; i < n; i++) {
        ...
        //L.add(intColor);
        L.add(Color.fromRGBO(R, G, B, 1.0));
      }
                          ...
                          IconButton(icon: isfavorite ? Icon(Icons.favorite,color: red,size: h*0.035,)
                              ...
                              onPressed: () async{
                                if(isfavorite){
                                  ...
                                  //await deletePalette(addedPalette);
                                  Provider.of<Palettes>(context, listen: false).delete(Provider.of<Palettes>(context, listen: false).listPalettes.last.id);
                                  ...
                                  /* addNewPalette(
                                      ...
                                      size: size.toString(),
                                    category_id:get_category_id(type).toString(),addedpalette:addedPalette );*/
                                  Provider.of<Palettes>(context, listen: false).add(listColors: listColor);
                                }
                                }
                          ),
                        ],
                    ...
                    Column(
                        children: [
                          ...
                          Container(
                            ...
                            child: FlatButton(
                              child: Text(
                                //i need a function that turn an RGB to hex
                                "#${fromRGBtoHex(selectedColor).toRadixString(16)}",
                                style: TextStyle(
                              ...
                              onPressed: () {
                                Toast.show(
                                  "${fromRGBtoHex(selectedColor).toRadixString(16)} copied to Clipboard",
                                  context,
                                  duration: 2,
                                  gravity: Toast.TOP,
                                  backgroundRadius: 30,
                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      if (val != null) {
                        var j = listColor.indexWhere((element) =>
                        element == selectedColor);
                        //j'ai un problème avec l'opacity du slider elle n'est pas sauveg rdé dans la list  des couleurs
                        //je pense il faut changer toutes les couleurs en RGB au liex des int, ou plutot comme c écrit dans la data
                        
                        int R = val.red;
                        int G = val.green;
                        int B = val.blue;
                        double alpha=val.opacity;
                        ...
                        intColor = int.parse(color, radix: 16);
                        setState(() {
                          listColor[j] = Color.fromRGBO(R, G, B, alpha);
                          selectedColor = Color.fromRGBO(R, G, B, alpha);
                        });
                        setState(() {
  ...
  void generateWarmColors(int n) {
    ...
    for (int i = 0; i < n; i++) {
      if (lockList[i] == false) {
        ...
        intColor = int.parse(color, radix: 16);
        L[i] = intColor;
       // L[i]=Color.fromRGBO(R, G, B, opacity)
      }
      setState(() {
        listColor = L;
      });
                  ...
                  onPressed: () {
                    showDialog(
                      ...
                      if (val != null) {
                        ...
                        String color;
                        int intColor;
                        color = componentToHex(a) +componentToHex(R) +
                            componentToHex(G) +
                            componentToHex(B);
                        intColor = int.parse(color, radix: 16);
                        setState(() {
                          ...
                          print("${intColor.toRadixString(16)}");
                          print("${intColor}");
                        });
                      }
                    });
